...
function Slackbot(configuration) {
    ...
    // pass RTM identity to webhook API via config object
    slack_botkit.setWebhookIdentity = function(identity) {

        slack_botkit.config.identity = typeof(identity) == 'object' ? identity : {};
            ...
            if (req.body.payload) {
                ...
                slack_botkit.findTeamById(message.team.id, function(err, team) {
                    if (err || !team) {
                        ...
                        bot.team_info = team;
                        bot.res = res;
                        bot.identity = slack_botkit.config.identity || {};
    ...
    slack_botkit.setupWebserver = function(port, cb, options = {}) {
        ...
        if (typeof options.ssl_certificates !== 'undefined') {
            server_setup = https.createServer({
                    key: fs.readFileSync(options.ssl_certificates.key),
                    cert: fs.readFileSync(options.ssl_certificates.cert),
                    ca: fs.readFileSync(options.ssl_certificates.ca_bundle)
                }, slack_botkit.webserver);
        } else {
            server_setup = slack_botkit.webserver;
        }

        var server = server_setup.listen(
        ...
        webserver.get('/oauth', function(req, res) {
            ...
            oauth_access(opts, function(err, auth) {
                ...
                if (err) {
                    ...
                    slack_botkit.log('Got auth', auth);
